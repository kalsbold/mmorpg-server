#pragma once
#include <gisunnet/transport/TcpTransport.h>
#include <gisunnet/buffers/ByteBuffer.h>


class NetTransport : public gisunnet::TcpTransport
{
public:
	NetTransport(boost::asio::io_service& io_service);
	~NetTransport();

	std::function<void()> MessageCallback;

protected:
	virtual int ProcessRead(gisunnet::ByteBuf& read_buf) override
	{
		DecodeMessage(read_buf);
...
NetTransport::NetTransport(boost::asio::io_service& io_service)
	: TcpTransport(io_service)
{
}
